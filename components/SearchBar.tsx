...
const SearchBar = ({
  ...
  // Cache keys and expiration (5 minutes)
  const SAVED_LOCATIONS_CACHE_KEY = (userId: string) => `savedLocations_${userId}`;
  const RECENT_LOCATIONS_CACHE_KEY = "recentLocations";
  const CACHE_EXPIRY_MS = 5 * 60 * 1000; // 5 minutes

  // Load cached saved locations
  const loadCachedSavedLocations = useCallback(async (userId: string) => {
    try {
      const cacheKey = SAVED_LOCATIONS_CACHE_KEY(userId);
      ...
      if (cached) {
        ...
        if (now - timestamp < CACHE_EXPIRY_MS) {
          if (__DEV__) {
            console.log('[SearchBar] ✅ Loaded saved locations from cache', {
              count: data.length,
          ...
          if (__DEV__) {
            console.log('[SearchBar] ⏰ Saved locations cache expired, will refresh');
          }
        }
      }
    } catch (error) {
      if (__DEV__) {
        console.warn('[SearchBar] Failed to load cached saved locations:', error);
      }
    }
    return null;
  }, []);

  // Save saved locations to cache
  const saveSavedLocationsToCache = useCallback(async (userId: string, locations: any[]) => {
    try {
      const cacheKey = SAVED_LOCATIONS_CACHE_KEY(userId);
      const cacheData = {
        data: locations,
      ...
      if (__DEV__) {
        console.log('[SearchBar] ✅ Cached saved locations', { count: locations.length });
      }
    } catch (error) {
      if (__DEV__) {
        console.warn('[SearchBar] Failed to cache saved locations:', error);
      }
    }
  }, []);

  // Fetch saved locations from API
  const fetchSavedLocations = useCallback(async (userId: string, useCache: boolean = true) => {
    try {
      // Try cache first if enabled
      if (useCache) {
        const cached = await loadCachedSavedLocations(userId);
        if (cached) {
          setSavedLocations(cached);
          // Refresh in background
          fetchSavedLocations(userId, false).catch(() => {
            // Silent failure - we have cached data
          });
          return;
        }
      }
      ...
      const mapped = response.data.map((loc: any) => ({
        ...
        address: loc.label,
        id: loc._id, // Preserve ID for future use
      }));
      
      setSavedLocations(mapped);
      
      // Save to cache
      await saveSavedLocationsToCache(userId, mapped);
      
      if (__DEV__) {
        console.log('[SearchBar] ✅ Fetched and cached saved locations', { count: mapped.length });
      }
    } catch (apiError) {
      console.error('[SearchBar] Failed to fetch saved locations:', apiError);
      ...
      if (useCache) {
        ...
        if (cached) {
          setSavedLocations(cached);
          if (__DEV__) {
            console.log('[SearchBar] ⚠️ Using cached saved locations due to API error');
          }
          return;
        }
      }
  ...
  }, [loadCachedSavedLocations, saveSavedLocationsToCache]);

  // Load recent locations from cache
  const loadRecentLocations = useCallback(async () => {
    try {
      const storedRecent = await AsyncStorage.getItem(RECENT_LOCATIONS_CACHE_KEY);
      if (storedRecent) {
        const parsed = JSON.parse(storedRecent);
        setRecentLocations(parsed);
        if (__DEV__) {
          console.log('[SearchBar] ✅ Loaded recent locations from cache', { count: parsed.length });
        }
      } else {
        setRecentLocations([]);
        if (__DEV__) {
          console.log('[SearchBar] No recent locations found in cache');
        }
      }
    } catch (storageError) {
      console.error('[SearchBar] Failed to load recent locations:', storageError);
      setRecentLocations([]);
    }
  }, []);

  // Memoized location loading function with caching
  const loadLocations = useCallback(async () => {
    try {
      if (__DEV__) {
        console.log('[SearchBar] Loading locations for userId:', userId);
      }
      
      // Load saved locations (with cache)
      if (userId) {
        await fetchSavedLocations(userId, true);
      } else {
        setSavedLocations([]);
      }

      // Load recent locations (always from cache)
      await loadRecentLocations();
    } catch (error) {
      console.error("[SearchBar] Failed to load locations:", error);
    }
  }, [userId, fetchSavedLocations, loadRecentLocations]);

  // Load locations on mount and when userId changes
  useEffect(() => {
    loadLocations();
  }, [loadLocations]);
  
  // Refresh saved locations when component becomes visible (if cache expired)
  useEffect(() => {
    const checkAndRefreshCache = async () => {
      ...
      try {
        ...
        if (cached) {
          ...
          if (now - timestamp >= CACHE_EXPIRY_MS) {
            if (__DEV__) {
              console.log('[SearchBar] 🔄 Cache expired, refreshing saved locations in background');
            }
    ...
    // Check cache expiry periodically (every 2 minutes)
    const interval = setInterval(checkAndRefreshCache, 2 * 60 * 1000);
    
    // Initial check
    checkAndRefreshCache();
    
    return () => clearInterval(interval);
  }, [userId, refreshSavedLocations]);
  ...
  // Memoized function to save to recent locations (with cache)
  const addToRecent = useCallback(async (place: { latitude: number; longitude: number; address: string }) => {
    try {
      ...
      recentList = recentList.filter((item: any) => 
      ...
      recentList.unshift({
        ...place,
        timestamp: Date.now(), // Add timestamp for future sorting
      });
      ...
      await AsyncStorage.setItem(RECENT_LOCATIONS_CACHE_KEY, JSON.stringify(recentList));
      setRecentLocations(recentList);
      
      if (__DEV__) {
        console.log('[SearchBar] ✅ Saved to recent locations cache', { count: recentList.length });
      }
    } catch (error) {
  ...
  // Refresh saved locations (force API fetch)
  const refreshSavedLocations = useCallback(async () => {
    if (userId) {
      if (__DEV__) {
        console.log('[SearchBar] 🔄 Refreshing saved locations...');
      }
      await fetchSavedLocations(userId, false);
    }
  }, [userId, fetchSavedLocations]);
  ...
  const MotorItem = useCallback(({ motor }: { motor: Motor }) => (
    ...
    >
      ...
      />
      <View style={styles.motorTextContainer}>
        {motor.nickname && (
          <Text 
            style={[
              styles.motorNickname,
              selectedMotor?._id === motor._id && styles.selectedMotorNickname
            ]}
            numberOfLines={1}
  ...
  const locationListComponent = useMemo(() => {
    ...
    console.log(`[SearchBar] Rendering ${activeTab} locations:`, locations);
    return locations.map((location, index) => (
      <TouchableOpacity
        key={`${activeTab}-${index}`}
        style={styles.locationItem}
        onPress={() => {
          console.log(`[SearchBar] ${activeTab} location pressed:`, location);
