...
export default function RouteSelectionScreen({ navigation, route }: RouteSelectionScreenProps) {
  ...
  // Debug render tracking (DIAGNOSTIC - Remove after fixing)
  ...
  const currentDeps = {
    ...
    gasStationsLength: gasStations?.length,
    motorsLength: motors?.length,
    cachedReportsLength: cachedReports?.length,
    cachedGasStationsLength: cachedGasStations?.length,
    cachedMotorsLength: cachedMotors?.length,
    isTracking: screenMode === 'tracking',
  };
  ...
  if (changedDeps.length > 0 || renderCountRef.current === 1) {
    console.log(`[RouteSelection] RENDER #${renderCountRef.current}`, {
      changedDeps: changedDeps.length > 0 ? changedDeps : 'initial render',
      values: changedDeps.reduce((acc, key) => {
        acc[key] = { 
